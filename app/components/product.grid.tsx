...
export function ProductGrid() {
	...
	return (
		<div className='max-w-7xl mx-auto mt-20 mb-10 px-16 md:px-20'>
			<h1 className='text-3xl font-bold text-center mb-8'>FEATURED PRODUCTS</h1>

			<div className='flex  gap-4 mb-8'>
				{['SHOW ALL', 'FITNESS', 'CYCLING', 'WINTER'].map(filter => (
					<Button key={filter} variant={activeFilter === filter ? 'default' : 'ghost'} onClick={() => setActiveFilter(filter)} className='text-sm'>
						{filter}
					</Button>
				))}
			</div>
			...
			<div className='grid grid-cols-1 sm:grid-cols-2  md:grid-cols-4 gap-6'>
				{filteredProducts.map((product, index) => (
					<div key={index} className='group relative border rounded-lg'>
						<div key={index} className='group relative  rounded'>
							...
							<div className='space-y-1 ml-3 mb-3'>
								...
								<div className='flex items-center  gap-32'>
									...
									<Button variant='outline' size='sm' onClick={() => setSelectedProduct(product)} disabled={product.isSoldOut}>
										Buy
									</Button>
								</div>
							</div>
